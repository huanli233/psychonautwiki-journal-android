...
class RemindersViewModel @Inject constructor(
    ...
    val reminders = experienceRepository.getAllRemindersFlow()
        .stateIn(
    ...
    fun deleteReminder(reminder: IngestionReminder) {
        viewModelScope.launch {
            notificationManager.cancelReminder(reminder.id)
            experienceRepository.delete(reminder)
        }
    }

    fun toggleReminder(reminder: IngestionReminder, isEnabled: Boolean) {
        viewModelScope.launch {
            val updated = reminder.copy(isEnabled = isEnabled)
            experienceRepository.update(updated)
            
            if (isEnabled) {
                notificationManager.scheduleReminder(updated)
            } else {
                notificationManager.cancelReminder(reminder.id)
            }
        }
